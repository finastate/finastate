...
pub trait ConsensusSupport: Send + Sync + 'static {
	...
	fn put_consensus_data<T: Encode>(
		&self,
		transaction: &mut DBTransaction,
		key: &[u8],
		value: T,
	) -> CommonResult<()>;
	fn delete_consensus_data(
		&self,
		transaction: &mut DBTransaction,
		key: &[u8],
	) -> CommonResult<()>;
...
impl ConsensusSupport for DefaultConsensusSupport {
	...
	fn put_consensus_data<T: Encode>(
		...
		self.chain.put_consensus_data(transaction, key, value)
	}
	fn delete_consensus_data(
		&self,
		transaction: &mut DBTransaction,
		...
		self.chain.delete_consensus_data(transaction, key)
	}
	fn commit_consensus_data(&self, transaction: DBTransaction) -> CommonResult<()> {
